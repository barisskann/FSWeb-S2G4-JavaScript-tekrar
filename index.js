...
ucetambolunenler = () => {
  const bosDizi = [];
  const render = sayilar.forEach((item) => {
    if (item % 3 === 0) {
      bosDizi.push(item);
    }
  });
...
ucebolunenlerintoplami = () => {
  const tamBolunenSayilar = ucetambolunenler();
  const render = tamBolunenSayilar.reduce(
    (toplam, sayac) => (toplam += sayac),
    0
  );
  return render;
};
...
besyuzdenkucuksayilar = () => {
  const render = sayilar.filter((item) => item < 500);
  return render;
};
besyuzdenkucuksayilar();
//3e çözümü
...
siralisayilar = () => {
  const besyuzkucuk = besyuzdenkucuksayilar();
  const render = besyuzkucuk.sort((a, b) => {
    return a - b;
  });
  return render; // sayilar[0,1,2,0,4,5,6,7]
};
//3f çözümü
tekraredensayilar = () => {
  const bosDizi = [];
